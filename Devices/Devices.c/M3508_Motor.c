...
Motor_Init_t M3508_Fric_Wheel[3];
...
void M3508_Fric_Wheel_Send_Data(int16_t Fric_Wheel_Left_Current,int16_t Fric_Wheel_Right_Current,int16_t Fric_Wheel_Bottom_Current);
...
void M3508_Fric_Wheel_Get_Data(CAN_Export_Data_t RxMessage)
{
	switch(RxMessage.CAN_RxHeader.StdId)
	{
		...
		case M3508_FRIC_WHEEL_RIGHT_ID:
			...
			break;
		case M3508_FRIC_WHEEL_BOTTOM_ID:
			M3508_Fric_Wheel[2].Actual_Angle = (int16_t)(RxMessage.CANx_Export_RxMessage[0] << 8 | RxMessage.CANx_Export_RxMessage[1]);
			M3508_Fric_Wheel[2].Actual_Speed = (int16_t)(RxMessage.CANx_Export_RxMessage[2] << 8 | RxMessage.CANx_Export_RxMessage[3]);
			M3508_Fric_Wheel[2].Actual_Current = (int16_t)(RxMessage.CANx_Export_RxMessage[4] << 8 | RxMessage.CANx_Export_RxMessage[5]);
			M3508_Fric_Wheel[2].Temperature = RxMessage.CANx_Export_RxMessage[6];
		
			M3508_Fric_Wheel[2].Info_Update_Frame++;
			break;
	}
}
...
void M3508_Fric_Wheel_Send_Data(int16_t Fric_Wheel_Left_Current,int16_t Fric_Wheel_Right_Current, int16_t Fric_Wheel_Bottom_Current)
{
	CAN_Func.CAN_0x200_Send_Data(&hcan1,Fric_Wheel_Left_Current,Fric_Wheel_Right_Current,Fric_Wheel_Bottom_Current,0);
}
